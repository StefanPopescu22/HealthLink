...
function ClinicCreateDoctor() {
  ...
  const [workingHours, setWorkingHours] = useState([{ weekday: 1, startTime: "09:00", endTime: "15:00" }]);
  const [formData, setFormData] = useState({ firstName: "", lastName: "", email: "", password: "", specialtyIds: [], experienceYears: "", description: "" });
  ...
  useEffect(() => {
    ...
    const loadClinicProfile = async () => {
      try {
        const profileResponse = await api.get("/clinic/me");
  ...
  const toggleSpecialty = (specialtyId) => {
    setFormData((prev) => {
      ...
      return { ...prev, specialtyIds: exists ? prev.specialtyIds.filter((id) => id !== specialtyId) : [...prev.specialtyIds, specialtyId] };
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post("/clinic/doctors", { ...formData, workingHours });
      setSuccess(response.data.message || "Doctor profile created successfully.");
      setFormData({ firstName: "", lastName: "", email: "", password: "", specialtyIds: [], experienceYears: "", description: "" });
            ...
            <section className="clinic-doctor-page">
              ...
              <form className="soft-card clinic-doctor-form-card" onSubmit={handleSubmit}>
                <div className="clinic-doctor-form-grid">
                  <div className="clinic-doctor-input-group">
                    ...
                    <div className={`clinic-doctor-input-wrapper ${fieldErrors.firstName ? 'has-error' : ''}`}>
                      <FaUser />
                      <input id="firstName" name="firstName" type="text" autoComplete="given-name" value={formData.firstName} onChange={handleChange} placeholder="e.g. Ion" />
                    </div>
                  ...
                  <div className="clinic-doctor-input-group">
                    ...
                    <div className={`clinic-doctor-input-wrapper ${fieldErrors.lastName ? 'has-error' : ''}`}>
                      <FaUser />
                      <input id="lastName" name="lastName" type="text" autoComplete="family-name" value={formData.lastName} onChange={handleChange} placeholder="e.g. Popescu" />
                    </div>
                  ...
                  <div className="clinic-doctor-input-group">
                    ...
                    <div className={`clinic-doctor-input-wrapper ${fieldErrors.email ? 'has-error' : ''}`}>
                      <FaEnvelope />
                      <input id="email" name="email" type="email" autoComplete="email" value={formData.email} onChange={handleChange} placeholder="doctor@clinic.com" />
                    </div>
                  ...
                  <div className="clinic-doctor-input-group">
                    ...
                    <div className={`clinic-doctor-input-wrapper ${fieldErrors.password ? 'has-error' : ''}`}>
                      <FaLock />
                      <input id="password" name="password" type="password" autoComplete="new-password" value={formData.password} onChange={handleChange} placeholder="Secure password" />
                    </div>
                  ...
                  <div className="clinic-doctor-input-group">
                    ...
                    <div className={`clinic-doctor-input-wrapper ${fieldErrors.experienceYears ? 'has-error' : ''}`}>
                      <FaUserDoctor />
                      <input id="experienceYears" name="experienceYears" type="number" min="0" autoComplete="off" value={formData.experienceYears} onChange={handleChange} placeholder="e.g. 10" />
                    </div>
                  ...
                  <div className="clinic-doctor-input-group full">
                    ...
                    <textarea id="description" name="description" autoComplete="off" value={formData.description} onChange={handleChange} placeholder="Brief description of the doctor's background and expertise..." />
                  </div>
                  <div className="clinic-doctor-input-group full">
                    ...
                    <div className="clinic-doctor-specialties-grid">
                      {specialties.map((specialty) => {
                        ...
                        return (
                          <label key={specialty.id} htmlFor={checkboxId} className={`clinic-doctor-specialty-pill ${isChecked ? "active" : ""}`}>
                            <input id={checkboxId} type="checkbox" name="specialtyIds" checked={isChecked} onChange={() => toggleSpecialty(specialty.id)} />
                            {specialty.name}
                          </label>
                        );
                      })}
                ...
                <div style={{ marginTop: '24px' }}>
                  <WorkingHoursEditor workingHours={workingHours} setWorkingHours={setWorkingHours} />
                </div>
                ...
                <button type="submit" className="primary-btn clinic-doctor-submit-btn">Create Doctor Profile</button>
              </form>
            </section>
          </div>
        </div>
