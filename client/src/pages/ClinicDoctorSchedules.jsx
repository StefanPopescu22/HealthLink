...
function ClinicDoctorSchedules() {
  ...
  const loadDoctors = async () => {
    try {
      const response = await api.get("/clinic/doctor-schedules/doctors");
  ...
  const loadSchedule = async (doctorId) => {
    ...
    try {
      const response = await api.get(`/clinic/doctor-schedules/${doctorId}`);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const response = await api.post(`/clinic/doctor-schedules/${selectedDoctorId}`, formData);
  ...
  const handleUpdate = async (scheduleId) => {
    ...
    try {
      const response = await api.put(`/clinic/doctor-schedules/${scheduleId}`, editingData);
  ...
  const handleDelete = async (scheduleId) => {
    ...
    try {
      const response = await api.delete(`/clinic/doctor-schedules/${scheduleId}`);
          ...
          <div className="dashboard-page-content">
            <section className="doctor-schedules-page">
              <div className="soft-card doctor-schedules-header">
                <h1>Doctor Working Hours</h1>
                <p>Manage weekly working intervals for your clinic doctors.</p>
              </div>
